            ...
            <div class="col-md-9">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        @endif
                        {{ Form::open(['method' => 'GET', 'url' => '/admin/user', 'class' => 'form-inline my-2 my-lg-0 float-right', 'user' => 'search'])  }}
                        ...
                        <div class="table-responsive">
                            <table class="table table-borderless">
                                <thead>
                                    <tr>
                                        <th>#</th><th>{{ __('name') }}</th><th>{{ __('email') }}</th><th>{{ __('actions') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                @foreach($user as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->email }}</td>
                                        <td>
                                            <a href="{{ url('/admin/user/' . $item->id) }}"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> {{ __('show') }}</button></a>
                                            <a href="{{ url('/admin/user/' . $item->id . '/edit') }}"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> {{ __('edit') }}</button></a>
                                            {{ Form::open([
                                                'method' => 'DELETE',
