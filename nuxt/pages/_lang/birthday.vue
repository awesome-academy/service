    ...
    <section class="section section-components">
      <div class="container">
        <div class="alert alert-primary mb-4" role="alert">
          <strong>{{ $t('birthday.list_title') }}:</strong>
        </div>
        <div v-for="item in items" :key="item.id" class="row mb-5">
          <div class="col-md-12 mb-5 mb-md-0">
            ...
            <div>
              ...
              <b class="">{{ $t('birthday.number') }}: <span class="text-success">{{ item.min_people }} - {{ item.max_people }} </span> {{ $t('birthday.people') }}</b>
              <br/>
              <b v-if="item.status.name === 'free'" class="">{{ $t('birthday.status') }}: <span class="text-success"> {{ $t('birthday.free') }} </span></b>
              <b v-else class="">{{ $t('birthday.status') }}: <span class="text-warning"> {{ $t('birthday.booking') }} </span></b>
            </div>
          </div>
        </div>
